...
describe('users integration tests', () => {
    test('GET /users/', async () => {
        ...
        expect(res.body[0].name).toBe('william')
    })
    ...
    test('POST /users/', async () => {
        const user = { name: 'bill', password: '123' }
    ...
    test('PUT /users/:id', async () => {
        const user = { name: 'will', password: '123' }
        const res = await supertest(server).post('/users/1').send(user)
        expect(res.statusCode).toBe(200)
        expect(res.type).toBe('application/json')
    ...
    test('DELETE /users/:id (404)', async () => {
        const res = await supertest(server).delete('/users/100')
        expect(res.statusCode).toBe(404)
    })
})